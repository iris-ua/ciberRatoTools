...
for h = 1:2
  ...
  if pp(h) < 2*tol
    per_str = 'none';                      % no significant oscillation
  else
    s  = sign(yw - mean(yw));
    ic = find(s(1:end-1) .* s(2:end) < 0); % mean crossings
    if numel(ic) >= 3
      per_str = sprintf('%.2f', 2 * mean(diff(tw(ic))));
    else
      per_str = sprintf('> %d', win);      % slower than the window
    end
  end
  fprintf('%-16s %10.3f %12.3f %12s %12.4f\n', names{h}, iae, pp(h), per_str, me);
end
fprintf('\n');
