...
int main(int argc, char **argv) {
   ...
   */
  WbDeviceTag left_motor = wb_robot_get_device("motor.left");
  WbDeviceTag right_motor = wb_robot_get_device("motor.right");
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    float left_speed = 0.4*MAX_SPEED; 
    float right_speed = 0.6*MAX_SPEED; 
    ...
    printf("%5.2f\t",get_bearing_in_rads(compass));
    printf("%5.2f\t",m);
    printf("%5.2f\t%5.2f\t",left_speed, right_speed);
